...
const tableName = 'base_repository_test';
...
describe('BaseRepository', () => {
  ...
  describe('findOne', () => {
    ...
    it('should find an existing Model by name', () =>
      expect(testRepository.findOne({ name })).to.eventually.have.property('id', testModel.id)
    );
    it('should reject when a non-existing name is used', () =>
      expect(testRepository.findOne({ name: 'non-existing-name' })).to.be.rejected);
  });

  describe('findById', () => {
    const name = 'findById-test';
    let testId;
    before(name, () =>
      testRepository
        .create({ name })
    ...
    it('should find an existing Model by id', () =>
      expect(testRepository.findById(testId)).to.not.be.rejected
    );
    it('should reject when a non-existing id is used', () =>
      expect(testRepository.findById(0)).to.be.rejected);
  });

  describe('findByName subclass method', () => {
    const name = 'findByName-test';
    let testModel;
    before(name, () =>
      testRepository
        .create({ name })
        .tap(model => { testModel = model; })
    );
    after(name, () => testRepository.deleteById(testModel.id));

    it('should find an existing Model by name', () =>
      expect(testRepository.findByName(name)).to.eventually.have.property('id', testModel.id)
    );
    it('should reject when a non-existing name is used', () =>
      expect(testRepository.findOne('non-existing-name')).to.be.rejected);
  });

  describe('findAll', () => {
    const name1 = 'findAll-1-test';
    const name2 = 'findAll-2-test';
    const testModels = [];
    before('setUp', () =>
      BPromise
        .all([name1, name2])
