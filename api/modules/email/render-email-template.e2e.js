...
describe('Email Template Service', () => {
  ...
  it('should render an existing template', () => {
    const result = renderEmailTemplate(testData, 'weekly-notifications');
    return Promise.all([
  ...
  it('should throw an error when trying to render a non-existing template', () =>
    expect(renderEmailTemplate(testData, 'non-existing-template')).to.be.rejectedWith(RenderError)
  );
});
